...
const Header = () => {
  ...
  return (
    <>
      <header className="sticky top-0 z-50 bg-gradient-to-r from-yellow-500 to-yellow-700 py-3 px-4 md:px-8 text-white shadow-lg transition-all duration-300">
        <nav className="flex justify-between items-center max-w-[1800px] mx-auto w-full">
          ...
          <Link to="/" className="flex items-center gap-3 no-underline group">
            <img
              ...
              className="w-9 h-9 rounded-full ring-2 ring-white/30 group-hover:ring-white transition-all duration-300 object-cover shadow-sm"
            />
          ...
          <div className="hidden md:flex items-center bg-white/20 hover:bg-white/30 focus-within:bg-white rounded-full px-4 py-2 flex-1 max-w-lg mx-8 transition-all duration-300 border border-white/30 focus-within:border-white shadow-inner group">
          ...
          <div className="hidden md:flex items-center gap-6">
            {/* Gold Rate Badge */}
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white/20 backdrop-blur-sm border border-white/30 rounded-full text-xs font-semibold text-white shadow-sm">
            ...
            {localStorage.getItem("authToken") && role === "user" && (
              <div className="flex items-center gap-5">
                ...
                <Link to="/cart" className="relative text-white hover:text-yellow-200 transition-colors flex flex-col items-center gap-0.5 group">
                  ...
                  {cartItemCount > 0 && (
                    <span className="absolute -top-1.5 -right-2 bg-red-500 text-white text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center border border-yellow-600 px-1 shadow-sm">
            ...
            {localStorage.getItem("authToken") ? (
              <div className="relative flex items-center gap-2 group pb-2 pt-2 cursor-pointer ml-2">
                <div className="w-9 h-9 rounded-full bg-white/20 flex items-center justify-center border-2 border-white/50 group-hover:border-white transition-all overflow-hidden shadow-sm">
                  {user && user.profile ? (
                ...
                {/* Dropdown Menu */}
                <div className="absolute right-0 top-full mt-1 w-56 bg-white rounded-xl shadow-xl overflow-hidden hidden group-hover:flex flex-col z-50 border border-gray-100 transition-all duration-300 transform origin-top-right text-gray-800">
              ...
              <Link to="/login" className="bg-white text-yellow-700 font-bold px-5 py-2 rounded-full hover:bg-yellow-50 hover:shadow-lg transition-all duration-300 shadow-md text-sm">
                Sign In
              </Link>
            )}
          </div>
          ...
          <div className="flex md:hidden items-center gap-4">
            {localStorage.getItem("authToken") && role === "user" && (
              <Link to="/cart" className="relative text-white flex items-center">
                ...
                {cartItemCount > 0 && (
                  <span className="absolute -top-1.5 -right-2 bg-red-500 text-white text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center border border-yellow-600">
                    {cartItemCount > 9 ? "9+" : cartItemCount}
                  </span>
                )}
              </Link>
      ...
      >
        ...
        {/* Drawer Content */}
        <div className="absolute top-0 left-0 h-full w-[85%] max-w-sm bg-white flex flex-col shadow-2xl overflow-y-auto">
          {/* Drawer Header */}
          <div className="bg-gradient-to-r from-yellow-500 to-yellow-700 py-5 px-5 flex justify-between items-center shadow-sm text-white">
            <div className="flex items-center gap-3 font-semibold text-lg">
              <img src="/logo.png" alt="Logo" className="w-8 h-8 rounded-full object-cover ring-2 ring-white/50" />
              <span className="font-serif font-bold drop-shadow-sm">Amit Jewellers</span>
            </div>
            <button
              onClick={() => setIsMobileMenuOpen(false)}
              className="text-white hover:text-yellow-200 transition-colors bg-white/20 rounded-full p-2"
            >
          ...
          <div className="flex-1 p-5 flex flex-col gap-6">
            ...
            {localStorage.getItem("authToken") && (
              <div className="flex items-center gap-4 pb-5 border-b border-gray-100">
                <div className="w-14 h-14 rounded-full bg-yellow-50 flex items-center justify-center border border-yellow-200 shadow-sm overflow-hidden text-yellow-700">
                  {user && user.profile ? (
                    <img src={user.profile} alt="Profile" className="w-full h-full object-cover" />
                  ) : (
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex bg-gray-50 rounded-xl border border-gray-200 overflow-hidden shadow-inner focus-within:bg-white focus-within:ring-2 focus-within:ring-yellow-400 focus-within:border-transparent transition-all">
                <input
                  type="text"
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="bg-yellow-50 rounded-xl p-4 border border-yellow-100 flex items-center gap-3">
            ...
            <div className="flex flex-col gap-1.5 mt-2">
              ...
              {role === "admin" && (
                <Link to="/create-product" className="flex items-center gap-3 p-3 rounded-lg text-gray-700 hover:bg-gray-50 hover:text-yellow-600 transition font-medium">
              ...
              {localStorage.getItem("authToken") && role === "user" && (
                <>
                  <Link to="/cart" className="flex items-center justify-between p-3 rounded-lg text-gray-700 hover:bg-gray-50 hover:text-yellow-600 transition font-medium">
                  ...
                  <Link to="/wishlist" className="flex items-center gap-3 p-3 rounded-lg text-gray-700 hover:bg-gray-50 hover:text-yellow-600 transition font-medium">
                  ...
                  <Link to="/profile" className="flex items-center gap-3 p-3 rounded-lg text-gray-700 hover:bg-gray-50 hover:text-yellow-600 transition font-medium">
                  ...
                  <Link to="/orders" className="flex items-center gap-3 p-3 rounded-lg text-gray-700 hover:bg-gray-50 hover:text-yellow-600 transition font-medium">
            ...
            <div className="mt-auto pt-6 border-t border-gray-100 pb-4">
              {localStorage.getItem("authToken") ? (
                <div onClick={() => setIsMobileMenuOpen(false)} className="w-full">
                  <SignOutButton className="flex items-center justify-center w-full bg-red-50 text-red-600 font-semibold py-3 rounded-xl hover:bg-red-100 transition border border-red-100" />
                </div>
              ) : (
                <Link to="/login" className="flex items-center justify-center w-full bg-gradient-to-r from-yellow-500 to-yellow-600 text-white font-semibold py-3 rounded-xl hover:from-yellow-600 hover:to-yellow-700 transition shadow-md">
                  Sign In
                </Link>
              )}
            </div>
